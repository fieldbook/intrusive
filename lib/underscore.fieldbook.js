...
var mixin = {
  ...
  // Map over an array using a soak path, with optional arguments to pass to the soak.
  // For example:
  //  var arr = [{x: {y: 'foo'}}, {x: {y: 'bar'}}]
  //  _.mapBySoak(arr, 'x.y')               // ['foo', 'bar']
  //  _.mapBySoak(arr, 'x.y.toUpperCase()') // ['FOO', 'BAR']
  //  _.mapBySoak(arr, 'x.y.slice', 1)      // ['oo', 'ar']

  mapBySoak: function (arr, path, vargs) {
    return _.fnBySoak('map', arguments);
  },

  // Filter an array using a soak path, with optional arguments to pass to the soak.
  // For example:
  //
  //  var arr = [
  //   {x: {y: 'turtle'}, z: true},
  //   {x: {y: 'puppet'}, z: false}
  //  ]
  //  _.filterBySoak(arr, 'x.z')                    // [{x: {y: 'turtle'}, z: 'true'}]
  //  _.filterBySoak(arr, 'x.y.startsWith', 'p')    // [{x: {y: 'puppet}, z: false}]
  filterBySoak: function (arr, path, vargs) {
    return _.fnBySoak('filter', arguments);
  },

  rejectBySoak: function (arr, path, vargs) {
    return _.fnBySoak('reject', arguments);
  },

  findBySoak: function (arr, path, vargs) {
    return _.fnBySoak('find', arguments);
  },

  findIndexBySoak: function (arr, path, vargs) {
    return _.fnBySoak('findIndex', arguments);
  },

  findLastIndexBySoak: function (arr, path, vargs) {
    return _.fnBySoak('findLastIndex', arguments);
  },

  everyBySoak: function (arr, path, vargs) {
    return _.fnBySoak('every', arguments);
  },

  someBySoak: function (arr, path, vargs) {
    return _.fnBySoak('some', arguments);
  },

  // Generic internal function for applying underscore methods with soak predicates
  // See mapBySoak and filterBySoak to understand how these are used
  fnBySoak: function (fnName, args) {
    var arr = args[0];
    var soakerArgs = _.rest(args);
    var soaker = _.soaker.apply(_, soakerArgs);
    return _[fnName](arr, soaker);
  },
  ...
  getRange: function (arr, start, stop) {
    return _.range(start, stop).map(function (i) { return arr[i]; });
  },
  ...
  logResult: function (tag, fn) {
    ...
    tag += ': '
    return function () {
      var result = fn.apply(this, arguments); // eslint-disable-line fieldbook/no-nested-this
  ...
  debugBefore: function (fn) {
    return function () {
      ...
      return fn.apply(this, arguments); // eslint-disable-line fieldbook/no-nested-this
    }
  },
  ...
  debugIf: function (value, condition) {
    ...
    if (_.isString(condition)) {
      /* jshint -W054 */
      condition = new Function('v', 'return v' + condition); // eslint-disable-line no-new-func
      /* jshint +W054 */
    }

    if (condition(value)) debugger; // eslint-disable-line no-debugger
    return value;
  },
...
// Aliases
mixin.pluckSoak = mixin.mapBySoak; // deprecated alias

mixin.allBySoak = mixin.everyBySoak;
mixin.anyBySoak = mixin.someBySoak;
mixin.selectBySoak = mixin.filterBySoak;
mixin.detectBySoak = mixin.findBySoak;
